...
export function DoctorCard({ doctor }) {
  ...
  const firstSpecialty = specialties[0] || "General";
      ...
      <CardContent>
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-1 mb-1">
              {specialties.map((spec, index) => (
                <Badge
                  key={index}
                  variant="outline"
                  className="bg-emerald-900/10 border-emerald-900/20 text-emerald-300 text-xs"
                >
                  {spec}
                </Badge>
              ))}
            ...
            >
              <Link
                href={`/doctors/${encodeURIComponent(firstSpecialty)}/${doctor.id}`}
              >
